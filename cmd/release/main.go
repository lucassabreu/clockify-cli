...
func main() {
	...
	newLink := fmt.Sprintf("[%s]: https://github.com/lucassabreu/clockify-cli/releases/tag/%s", newVersion, newVersion)
	lastLinkLine := findLastLinkLine(newContent)
	if lastLinkLine != -1 {
		linesList := strings.Split(newContent, "\n")
		var updatedLines []string
		updatedLines = append(updatedLines, linesList[:lastLinkLine+1]...)
		updatedLines = append(updatedLines, newLink)
		updatedLines = append(updatedLines, linesList[lastLinkLine+1:]...)
